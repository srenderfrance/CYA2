...
namespace cya2.Controllers
{
    ...
    public class DonorExportController : ControllerBase
    {
        ...
        public DonorExportController(IDataAccess data, IConfiguration config)
        {
            _data = data;
            _config = config;
        }
        ...
        public async Task<IActionResult> Export([FromQuery] string fund, [FromQuery] string start, [FromQuery] string end)
        {
            try
            {
                ...
                const string sql = "SELECT * FROM Donations WHERE Fund = @Fund AND Date >= @Start AND Date <= @End";
                var rows = await _data.LoadData<DonationsDataModel, dynamic>(sql, new { Fund = fund, Start = startDate, End = endDate }, _config.GetConnectionString("default") ?? string.Empty) ?? new List<DonationsDataModel>();
